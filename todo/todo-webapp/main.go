...
type Todo struct {
	...
	CreatedAt     time.Time
	UodatedAt     time.Time `bun:",nullzero"`
...
func main() {
	sqldb, err := sql.Open("postgres", "user=mugi password= host=localhost port=5432 dbname=todo sslmode=disable")
	if err != nil {
		log.Fatal(err)
	}
	defer sqldb.Close()
